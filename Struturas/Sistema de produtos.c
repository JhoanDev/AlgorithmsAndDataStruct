...
void addproduto(produtos *produto, int *i) // função para adicionar produto
{
    ...
    do
    {
        ...
        while (produto[*i].quantidade < MINQuantidade) //verificando se a quantidade é maior que zero
        {
        ...
        while (produto[*i].preco < MINPreco) //verificando se o valor é positivo
        {
            printf("Por favor insira uma preco valido: ");
            scanf("%f", &produto[*i].preco);
        }
        (*i)++;
        produto = realloc(produto, ((*i) + 1) * sizeof(produtos)); //realocando memoria
        if (produto == NULL)
        {
        ...
        printf("\n");
        system("cls");
    } while (cont != 2); //verificando se o usuario deseja cadastrar mais algum produto
}
...
void analiseprodutos(int *total, int *quantidades, float *valortotal, produtos *produto, int *i) // função para analisar e exibir os resultados
{
    ...
    for (j = 0; j < (*i); j++)
    {
        *quantidades += produto[j].quantidade; //quantidade total
        *valortotal += produto[j].preco * produto[j].quantidade; //preço total
    }
    *total = *i;
...
int main(void)
{
    produtos *produto = (produtos *)malloc(sizeof(produtos)); //alocando memoria dinamicamente
    if (produto == NULL)
    {
        printf("Erro na alocação de memoria, Fechando o programa...");
        exit(1);
    ...
    float valortotal = 0;
    do //exibindo tela inicial do programa
    {
    ...
    free(produto); //liberando espaços
    return 0;
}
